"""ImageFolder → SlipCache conversion verification.

These tests verify that:
1. JPEG images survive ImageFolder → SlipCache → load unchanged (byte-identical)
...
@pytest.fixture(scope="module")
def slipcache(imagefolder_reader):
    """Build SlipCache from ImageFolder.
    ...
    cache_dir = imagefolder_reader.cache_path
    _print_progress(f"\n[fixture] Dataset hash: {imagefolder_reader.dataset_hash}")
    ...
    _print_progress(f"[fixture] Building SlipCache from ImageFolder...")
...
@pytest.mark.s3
class TestImageFolderToSlipCacheStructure:
    """Verify SlipCache structure matches source."""

    def test_sample_count_matches(self, imagefolder_reader, slipcache):
        """Sample count should match source."""
        assert slipcache.num_samples == len(imagefolder_reader), \
            f"Count mismatch: cache={slipcache.num_samples}, source={len(imagefolder_reader)}"
...
@pytest.mark.s3
class TestImageFolderToSlipCacheQuick:
    """Quick verification of first 100 samples."""

    def test_first_100_bytes_comparison(self, imagefolder_reader, slipcache):
    ...
    def test_first_100_labels_match(self, imagefolder_reader, slipcache):
        """First 100 labels should match exactly."""
        _print_progress(f"\n  Comparing first 100 labels...")
        mismatches = []

        for idx in range(100):
            source_label = imagefolder_reader[idx]['label']
    ...
    def test_first_100_decodable(self, imagefolder_reader, slipcache):
...
@pytest.mark.s3
@pytest.mark.slow
class TestImageFolderToSlipCacheFull:
    """Full verification of all 50,000 samples."""

    def test_all_samples_format_check(self, imagefolder_reader, slipcache):
        """Check all samples for format issues (catches PNG-in-JPEG problem)."""
        _print_progress(f"\n  Checking all {len(imagefolder_reader)} samples for format issues...")
        image_format = slipcache.get_image_format('image')
        ...
        for idx in range(len(imagefolder_reader)):
            if idx % 5000 == 0:
                _print_progress(f"    {idx}/{len(imagefolder_reader)}...")
    ...
    def test_all_labels_match(self, imagefolder_reader, slipcache):
        """ALL labels should match exactly."""
        _print_progress(f"\n  Comparing all {len(imagefolder_reader)} labels...")
        mismatches = []
    ...
    def test_random_samples_pixel_match(self, imagefolder_reader, slipcache):
        """Random samples should decode to match source within tolerance."""
        import random
        random.seed(42)

        indices = random.sample(range(len(imagefolder_reader)), 100)
